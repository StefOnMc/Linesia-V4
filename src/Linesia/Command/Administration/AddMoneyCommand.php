...
class AddMoneyCommand extends Command {
    ...
    public function __construct() {
        parent::__construct("addmoney", "AddMoney - Linesia", "/Addmoney <player> <money>", ["addmoney"]);
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool {
        ...
        if (count($args) < 2){
            $sender->sendMessage(Utils::getPrefix() . "Usage : /addmoney <player> <money>.");
            return true;
        }
        ...
        EconomyAPI::getInstance()->addMoneyOnline($sender, $args[1]);
        $sender->sendMessage(Utils::getPrefix() . "Vous avez bien ajouté §9" . $args[1] . "$ §fà " . Utils::getRankPlayer($player) ."§f.");
        $player->sendMessage(Utils::getPrefix() . Utils::getRankPlayer($sender) . " §fvient de vous ajouter §9" . $args[1] . "$ §f.");
        Utils::sendDiscordLogs($sender->getName() . " vient d'ajouter " . $args[1] . "$ à " . $player->getName(), "**ADDMONEY**", 0x89EE01);
        return true;
    }
}