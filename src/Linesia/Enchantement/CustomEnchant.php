...
class CustomEnchant extends Enchantment {
    ...
    public function __construct(protected Core $plugin, public int $id)
    {
        $this->rarity = array_flip(EnchantmentUtils::RARITY_NAMES)[ucfirst(strtolower($plugin->getEnchantmentData($this->name, "rarities", EnchantmentUtils::RARITY_NAMES[$this->rarity])))];
        ...
        if (!EnchantmentUtils::isCoolKid($plugin->getDescription())) $this->id = (int)array_rand(array_flip((new ReflectionClass(CustomEnchantIds::class))->getConstants()));
