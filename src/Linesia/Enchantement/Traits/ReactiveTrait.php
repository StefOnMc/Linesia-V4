...
namespace Linesia\Enchantement\Traits;

use Linesia\Enchantement\CustomEnchant;
use Linesia\Enchantement\ReactiveEnchantment;
use Linesia\Utils\EnchantmentUtils;
use Linesia\Utils\ProjectileTracker;
...
trait ReactiveTrait{
    ...
    public static function attemptReaction(Player $player, Event $event): void
    {
        ...
        if ($event instanceof EntityDamageByChildEntityEvent || $event instanceof ProjectileHitBlockEvent) {
            ...
            if ($projectile instanceof Projectile && ProjectileTracker::isTrackedProjectile($projectile)) {
                if (!$event instanceof EntityDamageByEntityEvent || $event->getDamager() === $player) {
                    foreach (EnchantmentUtils::sortEnchantmentsByPriority(ProjectileTracker::getEnchantments($projectile)) as $enchantmentInstance) {
        ...
        foreach ($player->getInventory()->getContents() as $slot => $content) {
            foreach (EnchantmentUtils::sortEnchantmentsByPriority($content->getEnchantments()) as $enchantmentInstance) {
                /** @var ReactiveEnchantment $enchantment */
                $enchantment = $enchantmentInstance->getType();
                if ($enchantment instanceof CustomEnchant && $enchantment->canReact()) {
            ...
            foreach (EnchantmentUtils::sortEnchantmentsByPriority($content->getEnchantments()) as $enchantmentInstance) {
                ...
                if ($enchantment instanceof CustomEnchant && $enchantment->canReact()) {
                    if ((
                        ...
                        $enchantment->getUsageType() === CustomEnchant::TYPE_HELMET && EnchantmentUtils::isHelmet($content) ||
                        $enchantment->getUsageType() === CustomEnchant::TYPE_CHESTPLATE && EnchantmentUtils::isChestplate($content) ||
                        $enchantment->getUsageType() === CustomEnchant::TYPE_LEGGINGS && EnchantmentUtils::isLeggings($content) ||
                        $enchantment->getUsageType() === CustomEnchant::TYPE_BOOTS && EnchantmentUtils::isBoots($content)
                    )) {
                        foreach ($enchantment->getReagent() as $reagent) {
                            if ($event instanceof $reagent) {
                                $enchantmentStacks[$enchantment->getId()] = ($enchantmentStacks[$enchantment->getId()] ?? 0) + $enchantmentInstance->getLevel();
